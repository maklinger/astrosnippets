...
def saveFigure(fig, name, pdf=True, png=True, svg=False, dpi=400):
    ...
    if svg:
        if not os.path.exists(os.path.split("./figs_svg/" + name + ".svg")[0]):
            os.makedirs(os.path.split("./figs_svg/" + name + ".svg")[0])
        fig.savefig("./figs_svg/" + name + ".svg")
    if png:
...
def fixticks(ax, xmin, xmax, ymin, ymax, dx, dy, xstart=None, ystart=None, gridalpha=0.3):
    """sets the limits and ticks, adds grid and sets aspect ratio equal
